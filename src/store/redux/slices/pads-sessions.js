  ...
  reducers: {
    ...
    cleanupStaleData: (state, action) => {
      ...
      if (state?.padsSessionsCollection) {
        Object.entries(state?.padsSessionsCollection).forEach(
          ([id, document]) => {
...
const selectPadSession = (state) =>
  Object.values(state.padsSessionsCollection.padsSessionsCollection)[0]
    ?.sessions[0]?.notes;
