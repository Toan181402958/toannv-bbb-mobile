...
const usersSlice = createSlice({
  ...
  reducers: {
    addUser: (state, action) => {
      ...
      state.userIdMapCollectionId[action.payload.userObject.fields.userId] =
        userObject.id;
    },
    ...
    cleanupStaleData: (state, action) => {
      ...
      if (state?.usersCollection) {
        Object.entries(state?.usersCollection).forEach(([id, document]) => {
...
const selectUsers = (state) =>
  Object.values(state.usersCollection.usersCollection);
...
const selectUsersNoBreakout = (state) => {
  ...
  return allUsers.filter(
    (user) => user?.breakoutProps?.isBreakoutUser === false
  );
};

const selectMainUsers = createSelector([selectUsersNoBreakout], (allUsers) => {
  return allUsers;
});

const selectUserByIntId = (state, userId) =>
  selectUsers(state).find((user) => user.intId === userId);
...
export { selectUsers, selectUserByIntId, selectMainUsers, selectUsersName };
