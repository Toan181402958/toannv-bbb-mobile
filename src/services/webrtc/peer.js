...
import { stopStream, silentConsole } from './utils';
...
export default class WebRtcPeer extends EventEmitter2 {
  ...
  static internalLogger() {
    return {
      ...
      debug: (metadata, msg) =>
        WebRtcPeer.shimmedLogger('debug', msg, metadata),
      error: (metadata, msg) =>
        WebRtcPeer.shimmedLogger('error', msg, metadata),
    };
  }
      ...
      if (entry.candidate && entry.promise) {
        try {
          if (this.isPeerConnectionClosed()) {
            ...
            this.peerConnection
              .addIceCandidate(entry.candidate)
  ...
  _handleIceCandidate({ candidate }) {
    if (
      this.hasListeners('icecandidate') ||
      this.hasListeners('candidategatheringdone')
    ) {
      if (candidate) {
