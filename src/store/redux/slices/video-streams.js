  ...
  reducers: {
    ...
    cleanupStaleData: (state, action) => {
      ...
      if (state?.videoStreamsCollection) {
        Object.entries(state?.videoStreamsCollection).forEach(
          ([id, document]) => {
...
// Selectors
const selectVideoStreams = (state) =>
  Object.values(state.videoStreamsCollection.videoStreamsCollection);
  ...
  (videoStreams, users, localCameraId) => {
    return sortVideoUsers(
      users.map((user) => {
        ...
        const local =
          (typeof cameraId === 'string' && localCameraId === cameraId) ||
...
const videoStreamCleanupListener = (action, listenerApi) => {
  ...
  if (removedVideoStream?.stream)
    VideoManager.stopVideo(removedVideoStream.stream);
};
