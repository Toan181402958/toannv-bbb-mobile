...
const setupModules = (ws) => {
   ...
   */
  if (Settings.dev) {
    modules['external-video-meetings'] = new ExternalVideoMeetingsModule(
      messageSender
    );
  }
...
const SocketConnectionComponent = (props) => {
  ...
  const online = useSelector(
    (state) => state.client.connectionStatus.isConnected
  );
  const sessionEnded = useSelector((state) => state.client.sessionState.ended);
  const { meetingData, guestStatus } = useSelector((state) => state.client);
  const { loggedIn, loggingOut, loggingIn, connected } = useSelector(
    (state) => state.client.sessionState
  );
  ...
  const meetingEnded = useSelector(
    (state) => selectMeeting(state)?.meetingEnded
  );
  const basicSubscriptionsAreReady = useSelector((state) => {
    return (
      state.meetingCollection.ready &&
  ...
  const currentUserReady = useSelector(
    (state) => state.currentUserCollection.ready
  );
  ...
  useEffect(() => {
    if (
      ...
      ['meetings', 'users', 'guestUsers', 'record-meetings'].forEach(
        (module) => {
  ...
  const _terminate = (_loggingOut) => {
    ...
    if (
      _loggingOut &&
      meetingData &&
      typeof meetingData.logoutUrl === 'string'
    ) {
    ...
    switch (msgObj.msg) {
      ...
      case 'result': {
        ...
        if (msgObj.id === validateReqId.current) {
          // Session ended
          if (
            msgObj?.result?.reason &&
            TERMINATION_REASONS.some(
              (reason) => reason === msgObj.result.reason
            )
          ) {
            dispatch(
              sessionStateChanged({
      ...
      case 'changed': {
        ...
        if (msgObj.collection.includes('stream-external-videos')) {
          const SEVModule = new StreamExternalVideoModule(
...
export { getCurrentSessionId, getAuthInfo, makeCall, GLOBAL_MESSAGE_SENDER };
