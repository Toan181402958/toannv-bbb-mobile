...
const flushStoreEffect = (action, listenerApi) => {
  ...
  const hasEnded = (_state, type, payload) => {
    ...
    const disconnected =
      !_state.client.sessionState.connected ||
      (type === 'client/setConnected' && !payload);
    const ended =
      _state.client.sessionState.ended ||
      (type === 'client/sessionStateChanged' && payload.ended);
    const terminated =
      _state.client.sessionState.terminated ||
      (type === 'client/setSessionTerminated' && payload);
