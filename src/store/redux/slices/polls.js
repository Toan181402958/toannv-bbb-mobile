  ...
  reducers: {
    ...
    cleanupStaleData: (state, action) => {
      ...
      if (state?.pollsCollection) {
        Object.entries(state?.pollsCollection).forEach(([id, document]) => {
...
// Selectors
const selectActivePoll = (state) =>
  Object.values(state.pollsCollection?.pollsCollection)[0];
