...
class AudioBroker extends BaseBroker {
  constructor(wsUrl, role, options = {}) {
  ...
  getLocalStream() {
    if (
      this.webRtcPeer &&
      typeof this.webRtcPeer.getLocalStream === 'function'
    ) {
    ...
    return new Promise((resolve, reject) => {
      try {
        const options = {
          ...
          onicecandidate: !this.signalCandidates
            ? null
            : (candidate) => {
        ...
        if (typeof this.muted === 'boolean')
          this.setSenderTrackEnabled(!this.muted);
  ...
  scheduleReconnection() {
    const shouldSetReconnectionTimeout =
      !this._reconnectionTimeout && !this.started;
    ...
    if (shouldSetReconnectionTimeout) {
      const oldReconnectTimer =
        this._reconnectionTimer || BASE_RECONNECTION_TIMEOUT;
