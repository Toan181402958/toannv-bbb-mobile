  ...
  reducers: {
    ...
    cleanupStaleData: (state, action) => {
      ...
      if (state?.currentPollCollection) {
        Object.entries(state?.currentPollCollection).forEach(
          ([id, document]) => {
...
// Selectors
const selectCurrentPoll = (state) =>
  Object.values(state.currentPollCollection?.currentPollCollection)[0];
...
export { selectCurrentPoll, hasCurrentPollSelector };
