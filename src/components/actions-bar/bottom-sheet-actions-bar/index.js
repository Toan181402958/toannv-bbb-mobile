...
const BottomSheetActionsBar = ({ alwaysOpen }) => {
  ...
  const expandedActionsBar = useSelector(
    (state) => state.layout.expandActionsBar
  );
  const isModalShow = useSelector((state) => state.modal.isShow);

  const renderWithOpacity =
    route.name === 'FullscreenWrapperScreen' ||
  ...
  return (
    <BottomSheet
      ...
      handleIndicatorStyle={
        Styled[renderWithOpacity ? 'opacityStyles' : 'styles'].indicatorStyle
      }
      style={Styled[renderWithOpacity ? 'opacityStyles' : 'styles'].style}
      handleStyle={
        Styled[renderWithOpacity ? 'opacityStyles' : 'styles'].handleStyle
      }
      backgroundStyle={
        Styled[renderWithOpacity ? 'opacityStyles' : 'styles'].backgroundStyle
      }
      onChange={handleSheetChanges}
    >
      <View
        style={
          Styled[renderWithOpacity ? 'opacityStyles' : 'styles']
            .contentContainer
        }
      >
        <ActionsBar />
