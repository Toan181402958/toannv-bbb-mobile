...
const recordMeetingsSlice = createSlice({
  ...
  reducers: {
    ...
    removeRecordMeeting: (state, action) => {
      const { meetingObject } = action.payload;
      delete state.recordMeetingsCollection[meetingObject.id];
    },
    ...
    cleanupStaleData: (state, action) => {
      ...
      if (state?.recordMeetingsCollection) {
        Object.entries(state?.recordMeetingsCollection).forEach(
          ([id, document]) => {
...
const selectRecordMeeting = (state) =>
  Object.values(state.recordMeetingsCollection.recordMeetingsCollection)[0];
...
export { selectRecordMeeting, allowStartStopRecording, autoStartRecording };
