  ...
  reducers: {
    ...
    cleanupStaleData: (state, action) => {
      ...
      if (state?.currentUserCollection) {
        Object.entries(state?.currentUserCollection).forEach(
          ([id, document]) => {
...
const selectCurrentUser = (state) =>
  Object.values(state.currentUserCollection?.currentUserCollection)[0];
...
const logoutOrEjectionPredicate = (action, currentState) => {
  ...
  if (editCurrentUser.match(action) || addCurrentUser.match(action)) {
    const { currentUserObject } = action.payload;
    return (
      currentUserObject?.fields?.loggedOut || currentUserObject?.fields?.ejected
    );
  }
  ...
  if (addMeeting.match(action) || editMeeting.match(action)) {
    ...
    if (
      meetingEndedReason === 'BREAKOUT_ENDED_BY_MOD' ||
      meetingEndedReason === 'BREAKOUT_ENDED_EXCEEDING_DURATION'
    ) {
      if (isBreakoutUser) {
        return true;
      }
