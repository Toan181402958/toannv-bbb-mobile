...
import { AUDIO_GROUP_STREAMS_SUBSCRIPTION } from './queries';
import { SUBSCRIPTION_RETRY, ParticipantTypes } from './types';
...
export const useAudioSenders = (remoteParticipants) => {
  ...
  const groups = data?.user_audioGroup || [];
  const receiverFilter = [ParticipantTypes.RECEIVER, ParticipantTypes.SENDRECV];

  const myInboundGroupIds = groups
    .filter(
      (group) =>
        group.userId === currentUserId &&
        receiverFilter.includes(group.participantType)
    )
    .map((group) => group.groupId);

  const inAnyGroup = myInboundGroupIds.length > 0;

  const senderFilter = [ParticipantTypes.SENDER, ParticipantTypes.SENDRECV];
  ...
  const senders = groups
    ...
    .filter(
      (stream) => senderFilter.includes(stream.participantType) && stream.active
    );
...
export const useAudioSubscriptions = () => {
  ...
  const [subscriptionErrors, setSubscriptionErrors] = useState(new Map());
  ...
  const retrySubscription = useCallback((userId, publication) => {
    ...
    const delay =
      SUBSCRIPTION_RETRY.RETRY_INTERVAL **
  ...
  const handleSubscriptionChanges = useCallback(async () => {
    ...
    const desiredSubscriptions = new Set(senders.map((s) => s.userId));
                ...
                logger.debug(
                  {
                    ...
                    extraInfo: { userId: participantId, inAnyGroup },
                  },
