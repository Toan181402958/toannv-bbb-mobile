  ...
  reducers: {
    ...
    cleanupStaleData: (state, action) => {
      ...
      if (state?.voiceCallStatesCollection) {
        Object.entries(state?.voiceCallStatesCollection).forEach(
          ([id, document]) => {
...
const voiceCallStateChangePredicate = (action, currentState) => {
  if (!editVoiceCallState.match(action) && !addVoiceCallState.match(action))
    return false;
  const { voiceCallStateObject } = action.payload;
  ...
  // Not for us - skip
  if (
    currentVoiceCallState.userId !== AudioManager.userId ||
    // eslint-disable-next-line eqeqeq
    currentVoiceCallState.clientSession !=
      AudioManager.getCurrentAudioSessionNumber()
  ) {
    return false;
  }
...
const voiceCallStateChangeListener = (action, listenerApi) => {
  ...
  if (
    currentVoiceCallState?.callState !== previousVoiceCallState?.callState &&
    // eslint-disable-next-line eqeqeq
    ...
    switch (currentVoiceCallState?.callState) {
      case 'IN_CONFERENCE':
        ...
        break;
      case 'CALL_ENDED':
        if (currentState.audio.isConnected || currentState.audio.isConnecting) {
