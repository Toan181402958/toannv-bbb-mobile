...
class BaseBroker extends EventEmitter2 {
  ...
  constructor(
    ...
    this.logger = logger;
    this.reconnectCondition =
      typeof reconnectCondition === 'function'
  ...
  getRemoteStream() {
    if (
      this.webRtcPeer &&
      typeof this.webRtcPeer.getRemoteStream === 'function'
    ) {
  ...
  processOffer(sfuResponse) {
    if (this._validateStartResponse(sfuResponse)) {
      this.webRtcPeer
        .processOffer(sfuResponse.sdpAnswer)
  ...
  processAnswer(sfuResponse) {
    if (this._validateStartResponse(sfuResponse)) {
      this.webRtcPeer
        .processAnswer(sfuResponse.sdpAnswer)
        .then(this._processRemoteDescription.bind(this))
        .catch((error) => {
          this.logger.error(
            {
  ...
  handleConnectionStateChange(eventIdentifier) {
    if (this.webRtcPeer) {
      ...
      if (eventIdentifier) {
        const { eventName, detail } = notifyStreamStateChange(
          eventIdentifier,
          connectionState
        );
