  ...
  reducers: {
    ...
    cleanupStaleData: (state, action) => {
      ...
      if (state?.guestUsersCollection) {
        Object.entries(state?.guestUsersCollection).forEach(
          ([id, document]) => {
...
const selectWaitingUsers = createSelector(
  ...
  (guestUsersCollection) => {
    return Object.values(guestUsersCollection).filter(
      (guest) => !guest.approved && !guest.denied
    );
  }
);
...
export { selectGuestUserByDocumentId, selectWaitingUsers };
