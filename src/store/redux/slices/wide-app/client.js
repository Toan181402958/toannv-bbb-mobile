  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(join.fulfilled, (state, action) => {
        ...
        const joinGuestStatus = joinResponse.waitingForApproval
          ? 'WAIT'
          : 'ALLOW';
  ...
  async ({ url, logger }, thunkAPI) => {
    ...
    if (joinResponseUrl.includes('html5client/join')) {
      try {
        ...
        const { response: enterResponse } = await _fetchRetry(enterUrl, {
          method: 'GET',
        }).then((r) => r.json());
        ...
        throw new Error('/enter returncode is not SUCCESS', {
...
const leave = createAsyncThunk('client/leave', async (api, thunkAPI) => {
  // TODO check if already left :)
  const forceLeave = () => {
    const currentState = thunkAPI.getState();
    if (
      currentState.client.sessionState.ended &&
