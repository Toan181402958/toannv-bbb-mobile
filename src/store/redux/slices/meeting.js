  ...
  reducers: {
    ...
    cleanupStaleData: (state, action) => {
      ...
      if (state?.meetingCollection) {
        Object.entries(state?.meetingCollection).forEach(([id, document]) => {
...
const selectMeeting = (state) =>
  Object.values(state.meetingCollection.meetingCollection)[0];

const selectProp = (state, prop) => prop;

const selectAllUsersProps = createSelector([selectMeeting], (meeting) => {
...
const selectAllMetadata = createSelector([selectMeeting], (meeting) => {
